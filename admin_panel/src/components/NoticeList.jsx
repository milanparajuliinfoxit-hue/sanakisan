...
import { Edit, Trash2, Bell, ChevronLeft, ChevronRight, Search, ChevronUp, ChevronDown, ChevronsUpDown } from "lucide-react";
...
const NoticeList = ({ currentNotices = [], handleEdit, handleDelete, loading }) => {
  ...
  const filtered = useMemo(() => {
    ...
    return currentNotices.filter(n =>
      ...
      n.publishStatus?.toLowerCase().includes(q)
    );
  ...
  if (!currentNotices.length) {
    return (
      <div className="rounded-xl border border-border bg-card shadow-sm flex flex-col items-center justify-center py-20 text-center px-6">
        ...
        <p className="text-sm text-muted-foreground mt-1.5 max-w-xs">Create your first notice to get started.</p>
      </div>
    );
  }
  ...
  return (
    <>
      {isDialogOpen && (
        <AlertDialog
          onSubmit={() => { handleDelete(noticeId); setIsDialogOpen(false); setNoticeId(null); }}
          onCancel={() => { setIsDialogOpen(false); setNoticeId(null); }}
          ...
          <table className="w-full min-w-[800px]">
            ...
            <tbody className="divide-y divide-border">
              {paginated.length === 0 ? (
                ...
                <tr key={notice.id} className="hover:bg-muted/30 transition-colors duration-150">
                  <td className="px-4 py-3">
                    <div className="w-10 h-10 rounded-lg overflow-hidden border border-border bg-muted flex-shrink-0">
                      <CustomImage src={notice.featuredImage} className="w-full h-full object-cover" />
                    </div>
                  </td>
                  <td className="px-4 py-3 max-w-[220px]">
                    <p className="text-sm font-medium text-foreground truncate" title={notice.title}>{notice.title}</p>
                  </td>
                  <td className="px-4 py-3">
                    <p className="text-sm text-muted-foreground whitespace-nowrap">{notice.author}</p>
                  </td>
                  <td className="px-4 py-3">
                    <StatusBadge status={notice.publishStatus} />
                  </td>
                  <td className="px-4 py-3">
                    <p className="text-sm text-muted-foreground whitespace-nowrap">
                      {notice.publishDate ? new Date(notice.publishDate).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" }) : "—"}
                    </p>
                  </td>
                  <td className="px-4 py-3">
                    <p className="text-sm text-muted-foreground whitespace-nowrap">
                      {notice.createdAt ? new Date(notice.createdAt).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" }) : "—"}
                    </p>
                  </td>
                  <td className="px-4 py-3">
                    <p className="text-sm text-muted-foreground whitespace-nowrap">
                      {notice.updatedAt ? new Date(notice.updatedAt).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" }) : "—"}
                    </p>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-1.5">
                      <button onClick={() => handleEdit(notice)} title="Edit"
                        className="p-1.5 rounded-lg text-muted-foreground hover:text-blue-600 hover:bg-blue-50 transition-all duration-150">
                        <Edit className="w-4 h-4" />
                      </button>
                      <button onClick={() => { setNoticeId(notice.id); setIsDialogOpen(true); }} title="Delete"
