...
const CreateNotice = ({ selectedNotice, mode, setMode, onSuccess }) => {
  const { control, handleSubmit, setValue, reset } = useForm({
    defaultValues: { title: "", featuredImage: "", content: "", publishStatus: "", author: "", publishDate: "" },
  });
  ...
  const onSubmit = async (data, publishStatus) => {
    ...
    if (mode === "update") formData.append("id", selectedNotice?.id);

    try {
      const response = mode === "update"
        ? await updateNotice(formData).unwrap()
        : await saveNotice(formData).unwrap();
      if (response?.status) {
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-border bg-muted/30">
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Enter the main details of the notice</p>
          </div>
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="space-y-1.5">
              ...
              <Controller name="author" control={control} rules={{ required: "Author is required" }}
                render={({ field, fieldState: { error } }) => (
                  <>
            ...
            <div className="space-y-1.5">
              ...
              <Controller name="publishDate" control={control} rules={{ required: "Publish Date is required" }}
                render={({ field, fieldState: { error } }) => (
                  <>
        ...
        <div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden">
          ...
          <div className="p-6">
            <Controller name="featuredImage" control={control}
              render={({ field, fieldState: { error } }) => (
                <div>
                  ...
                  {showImage ? (
                    ...
                    <div
                      ...
                      className={cn(
                        "border-2 border-dashed rounded-xl cursor-pointer flex flex-col items-center justify-center h-48 transition-all duration-200 select-none",
        ...
        <div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-border bg-muted/30">
            <h3 className="font-semibold text-foreground text-sm uppercase tracking-wide">Content</h3>
            <p className="text-xs text-muted-foreground mt-0.5">Write the main body of the notice</p>
          </div>
          <div className="p-6">
        ...
        <div className="flex items-center justify-between pt-2 pb-6">
          ...
          <div className="flex items-center gap-3">
            <Button type="button" variant="outline" onClick={handleDraftSubmit}
              disabled={!!submitting} className="gap-2 min-w-[130px] h-10">
            ...
            <Button type="button" onClick={handlePublishSubmit}
              disabled={!!submitting} className="gap-2 min-w-[120px] h-10">
