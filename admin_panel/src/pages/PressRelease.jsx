...
const PressRelease = () => {
  ...
  const fetchPressReleases = async () => {
    setLoading(true);
    try {
      const query = new URLSearchParams({ page: 1, limit: ITEMS_PER_PAGE, publish_status: "", status: 1 });
  ...
  const handleDelete = async (pressReleaseId) => {
    try {
      ...
      if (response?.data?.status) fetchPressReleases();
    } catch (error) {
      throw new Error(error?.message);
    }
  };

  useEffect(() => { fetchPressReleases(); }, [getPress]);
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <Button
          onClick={() => { setMode(mode === "view" ? "create" : "view"); if (mode !== "view") setSelectedPressRelease(null); }}
          variant={mode === "view" ? "default" : "outline"}
          className="gap-2"
        >
          {mode === "view" ? <><Plus className="w-4 h-4" /> Create Press Release</> : <><List className="w-4 h-4" /> Press Releases List</>}
        </Button>
      </div>

      {mode === "view" ? (
        <PressReleaseList
          currentPressReleases={pressReleases}
