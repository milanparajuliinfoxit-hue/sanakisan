...
const Events = () => {
  ...
  const handleDelete = async (eventId) => {
    try {
      ...
      if (response?.data?.status) fetchEvent();
    } catch (error) {
  ...
  useEffect(() => { fetchEvent(); }, [getEventPagination]);
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <Button
          onClick={() => { setMode(mode === "view" ? "create" : "view"); if (mode !== "view") setEditSelectedItem(null); }}
          variant={mode === "view" ? "default" : "outline"}
          className="gap-2"
        >
          {mode === "view" ? <><Plus className="w-4 h-4" /> Create Event</> : <><List className="w-4 h-4" /> Events List</>}
        </Button>
      </div>
