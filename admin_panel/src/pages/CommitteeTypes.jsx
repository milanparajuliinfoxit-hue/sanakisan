...
} from "@/redux/api/committeeTypeApi";
...
function FormModal({ mode, item, onClose, onSuccess }) {
  ...
  const [createCommitteeType, { isLoading: creating }] = useCreateCommitteeTypeMutation();
  const [updateCommitteeType, { isLoading: updating }] = useUpdateCommitteeTypeMutation();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === "edit") {
        const res = await updateCommitteeType({ id: item.id, name: name.trim(), status }).unwrap();
        toast.success(res.message);
        onSuccess(res.data);
      } else {
        const res = await createCommitteeType({ name: name.trim() }).unwrap();
        toast.success(res.message);
        onSuccess(res.data);
      }
    } catch (err) {
      ...
      <div className="bg-card rounded-2xl shadow-2xl w-full max-w-md border border-border">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <h2 className="text-base font-semibold text-foreground">
            {mode === "edit" ? "Edit Committee Type" : "Create Committee Type"}
          </h2>
      ...
      <div className="bg-card rounded-2xl shadow-2xl w-full max-w-md border border-border overflow-hidden">
        <div className="p-6">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="text-base font-semibold text-foreground">Delete Committee Type?</h3>
...
// ── Main Page ─────────────────────────────────────────────────────────────────
export default function CommitteeTypes() {
  ...
  const [deleteCommitteeType] = useDeleteCommitteeTypeMutation();
  ...
  const { data: res, isLoading, refetch } = useGetCommitteeTypesQuery({
    page,
    limit: 10,
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await deleteCommitteeType(deleteTarget.id).unwrap();
      toast.success(res.message);
      refetch();
    } catch (err) {
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Committee Types</h1>
      ...
      <div className="grid grid-cols-2 gap-4">
        {[
          { label: "Total Types", value: totalItems, icon: Users2, color: "bg-primary/10 text-primary" },
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center px-6">
            ...
            <h3 className="text-lg font-semibold text-foreground">
              {search ? "No Types Found" : "No Committee Types Yet"}
            </h3>
            <p className="text-sm text-muted-foreground mt-1.5 max-w-xs">
              {search ? `No types match "${search}".` : "Create your first committee type to get started."}
            </p>
            {!search && (
              <button
                onClick={() => setFormModal({ mode: "create" })}
